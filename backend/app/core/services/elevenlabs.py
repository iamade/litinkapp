...
class ElevenLabsService:
    ...
    async def generate_enhanced_speech(
        ...
        try:
            ...
            public_url = await storage_service.upload(
                file_content, storage_path, "audio/mpeg"
            )
    ...
    async def generate_sound_effect(
        ...
        try:
            ...
            result = await self.generate_enhanced_speech(
                placeholder_text, "21m00Tcm4TlvDq8ikWAM", user_id
            )
            ...
            if result and result.get("audio_url"):
                ...
                return (
                    f"https://example.com/sound-effects/{effect_type}_{intensity}.mp3"
                )
    ...
    async def mix_audio_tracks(
        self, audio_tracks: List[Dict[str, Any]], user_id: str = None
    ) -> Dict[str, Any]:
    ...
    async def create_audio_narration(
        ...
        try:
            ...
            # Generate main narration
            narration_result = await self.generate_enhanced_speech(
                text=text, voice_id=voice_settings["voice_id"], user_id=user_id
            )
            ...
            if background_music:
                mixed_audio = await self.mix_audio_tracks(
                    audio_tracks=[
                        {"url": narration_result["audio_url"]},
                        {"url": background_music},
                    ],
                    user_id=user_id,
                )
                return mixed_audio
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/voices", headers={"xi-api-key": self.api_key}
                )
    ...
    async def _generate_speech_with_settings(
        self, text: str, voice_id: str, emotion: str = "neutral", speed: float = 1.0
    ) -> Optional[bytes]:
    ...
    async def _generate_speech(self, text: str, voice_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            print(
                f"ElevenLabs request - voice_id: {voice_id}, text_length: {len(text)}"
            )
    ...
    async def _download_and_mix_audio(
