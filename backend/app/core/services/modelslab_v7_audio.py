...
class ModelsLabV7AudioService:
    ...
    async def generate_tts_audio(
        ...
        try:
            ...
            logger.info(
                f"[MODELSLAB V7 TTS] Using voice: {voice_id}, model: {model_id}"
            )

            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.tts_endpoint, json=payload, headers=self.headers
                ) as response:
                    result = await response.json()

                    logger.info(
                        f"[MODELSLAB V7 TTS] Response status: {response.status}"
                    )
                    ...
                    if response.status == 200:
                        ...
                        raise Exception(
                            f"ModelsLab V7 TTS API error: {response.status} - {error_text}"
                        )
    ...
    async def generate_sound_effect(
        ...
        try:
            ...
            logger.info(
                f"[MODELSLAB V7 SFX] Generating sound effect: {description[:50]}..."
            )
            logger.info(f"[MODELSLAB V7 SFX] Duration: {duration}s, Model: {model_id}")

            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.sound_effects_endpoint, json=payload, headers=self.headers
                ) as response:
                    result = await response.json()

                    logger.info(
                        f"[MODELSLAB V7 SFX] Response status: {response.status}"
                    )
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        raise Exception(
                            f"ModelsLab V7 Sound Effects API error: {response.status} - {error_text}"
                        )
    ...
    async def generate_background_music(
        self, description: str, model_id: str = "music_v1", duration: float = 30.0
    ) -> Dict[str, Any]:
        """Generate background music using ModelsLab V7 API"""

        try:
            payload = {"prompt": description, "model_id": model_id, "key": self.api_key}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.music_endpoint, json=payload, headers=self.headers
                ) as response:
                    result = await response.json()

                    logger.info(
                        f"[MODELSLAB V7 MUSIC] Response status: {response.status}"
                    )
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        raise Exception(
                            f"ModelsLab V7 Music API error: {response.status} - {error_text}"
                        )
    ...
    def _process_audio_response(
        self, response: Dict[str, Any], audio_type: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"[MODELSLAB V7 {audio_type.upper()}] Response processing error: {e}"
            )
            return {
                "status": "error",
                "error": str(e),
    ...
    def get_available_models(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            ...
            "sound_effects": {"eleven_sound_effect": "ElevenLabs Sound Effects Model"},
            "music": {"music_v1": "Music Generation Model V1"},
        }
    ...
    async def batch_generate_tts(
        ...
        character_voice_list = (
            list(self.character_voices.values())
        ...
        for i, text_item in enumerate(texts):
            try:
                ...
                voice_id = text_item.get(
                    "voice_id", character_voice_list[i % len(character_voice_list)]
                )
                character = text_item.get("character", f"Speaker_{i+1}")

                logger.info(
                    f"[BATCH TTS] Processing {i+1}/{len(texts)}: {character} (voice: {voice_id})"
                )

                result = await self.generate_tts_audio(text=text, voice_id=voice_id)
    ...
    async def enhance_audio_for_scene(
        ...
        try:
            logger.info(
                f"[SCENE AUDIO] Enhancing audio for scene: {scene_description[:50]}..."
            )
            ...
            if include_ambient:
                ...
                ambient_result = await self.generate_sound_effect(
                    description=ambient_prompt, duration=15.0
                )
