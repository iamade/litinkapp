...
class ScriptModelRouter:
    """
    Intelligent model routing with automatic fallback and cost optimization.
    Uses ProviderRouter to route requests to an explicitly configured provider
    ...
    async def generate_script(
        ...
        result = await fallback_manager.try_with_fallback(
            service_type="script",
            user_tier=tier_str,
        ...
        telemetry = {
            ...
            "tier": tier_str,
            "ladder": config.models,
            "resolved_model": result.get("model_used"),
            "attempts": result.get("attempts", 0),
            "latency_ms": round((time.monotonic() - started_at) * 1000, 2),
            "success": result.get("status") == "success",
        }
        if result.get("status") != "success":
            telemetry["error"] = result.get("error", "unknown")
        logger.info("[ScriptModelRouter] telemetry %s", json.dumps(telemetry))
    ...
    async def _execute_generation(
        ...
        # Log the request
        logger.info(
            f"[ScriptModelRouter] Generating {script_type} script with {model} for tier {tier_str}"
        )
        ...
        try:
            ...
            if hasattr(response, "choices") and response.choices:
                ...
                logger.info(
                    f"[ScriptModelRouter] Successfully parsed response for {model}"
                )
            else:
                ...
                if hasattr(response, "json"):
                    try:
                        ...
                        logger.error(
                            f"[ScriptModelRouter] Failed to parse JSON response: {str(json_error)}"
                        )
                        raise ValueError(
            ...
            logger.error(
                f"[ScriptModelRouter] Failed to parse response for {model}: {str(e)}"
            )
    ...
    async def analyze_content(
        ...
        if system_prompt_override is not None:
            ...
            max_tokens = (
                ...
                else (config.max_tokens if config.max_tokens else 4000)
            )
            temperature = 0.7
    ...
    async def _execute_analysis(
        ...
        try:
            ...
            result_content = re.sub(
                r"<think[^>]*>.*?</think>", "", result_content, flags=re.DOTALL
            ).strip()
            if not result_content:
                logger.warning(
                    f"[ScriptModelRouter] Model {model} returned only thinking tags (no actual content) for {analysis_type}"
                )
                raise Exception(
                    f"Model {model} returned thinking-only response for {analysis_type}"
                )
            ...
            logger.error(f"[ScriptModelRouter] Analysis error with {model}: {str(e)}")
...
If expanding a specific section, seamlessly integrate new content so it reads as one cohesive piece.""",
    ...
    async def get_available_models(self) -> Dict[str, Any]:
        """
        Get the configured script-model ladder without calling a default provider.
        """
        models = []
        seen = set()
        for config in SCRIPT_MODEL_CONFIG.values():
            for model in config.models:
                if model not in seen:
                    seen.add(model)
                    models.append({"id": model, "name": model, "supported": True})
        return {"status": "success", "models": models}


class CostTracker:
    """Track script-model usage and estimated provider cost."""
