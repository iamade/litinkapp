...
class ChatterboxTTSProvider(TTSProvider):
    ...
    async def synthesize(self, text: str, voice_id: Optional[str] = None, model: Optional[str] = None, **kwargs: Any) -> TTSResult:
        if not settings.CHATTERBOX_API_KEY:
            return TTSResult(status="error", provider=self.provider_name, model=model or "default", voice_id=voice_id, error="CHATTERBOX_API_KEY is not configured")
        return TTSResult(status="error", provider=self.provider_name, model=model or "default", voice_id=voice_id, characters_used=len(text or ""), estimated_cost=self.get_cost_estimate(text, model=model), error="Chatterbox provider not fully implemented yet")
