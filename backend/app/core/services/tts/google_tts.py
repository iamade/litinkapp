...
class GoogleTTSProvider(TTSProvider):
    ...
    async def synthesize(self, text: str, voice_id: Optional[str] = None, model: Optional[str] = None, **kwargs: Any) -> TTSResult:
        if not settings.GOOGLE_TTS_API_KEY and not settings.GOOGLE_AI_STUDIO_API_KEY:
            return TTSResult(status="error", provider=self.provider_name, model=model or "text-to-speech", voice_id=voice_id, error="Google TTS API key is not configured")
        return TTSResult(status="error", provider=self.provider_name, model=model or "text-to-speech", voice_id=voice_id, characters_used=len(text or ""), estimated_cost=self.get_cost_estimate(text, model=model), error="Google TTS provider not fully implemented yet")
