...
class AIService:
    ...
    async def generate_quiz(self, content: str, difficulty: str = "medium", provider: str = "auto") -> List[Dict[str, Any]]:
    ...
    async def generate_lesson(self, content: str, topic: str, provider: str = "auto") -> Dict[str, Any]:
...
"""
            ...
            response = await asyncio.wait_for(
                self._make_completion(
                    messages=messages,
                    provider=provider,
                    response_format={"type": "json_object"} if provider != "deepseek" else None,  # DeepSeek might not support this
                    temperature=0.3,
                ),
    ...
    def generate_chapters_from_content_sync(
        ...
        return asyncio.run(self.generate_chapters_from_content(content, book_type, provider))

    async def generate_chapter_ai_elements(self, content: str, book_type: str, difficulty: str, provider: str = "auto") -> Dict[str, Any]:
        """Generate structured AI elements for a chapter (quizzes, key concepts, etc.), but not video scripts."""
        if book_type == "learning":
            return {
                "quiz_questions": await self.generate_quiz(content, difficulty, provider),
                "key_concepts": await self._extract_key_concepts(content, provider),
                "learning_objectives": await self._generate_learning_objectives(content, provider)
            }
        else:  # entertainment
    ...
    async def _extract_key_concepts(self, content: str, provider: str = "auto") -> List[str]:
        """Extract key concepts from content"""
        try:
            response = await self._make_completion(
                messages=[
                    {"role": "system", "content": "Extract 3-5 key concepts from this content. Return as a comma-separated list."},
    ...
    async def _generate_learning_objectives(self, content: str, provider: str = "auto") -> List[str]:
        """Generate learning objectives"""
        try:
            response = await self._make_completion(
                messages=[
                    {"role": "system", "content": "Generate 2-4 learning objectives based on this content. Each should start with an action verb."},
                    {"role": "user", "content": content[:1500]}
                ],
                provider=provider,
                temperature=0.4,
            ...
            objectives = [obj.strip('- ').strip() for obj in objectives_text.split('\n') if obj.strip()]
    ...
    async def generate_book_metadata(
        ...
        try:
            ...
            response = await self._make_completion(
                messages=[
                    {"role": "system", "content": "You are a book metadata analyst. Generate comprehensive book metadata."},
                    {"role": "user", "content": prompt}
                ],
                provider=provider,
                temperature=0.5,
    ...
    def generate_chapter_summary_sync(self, content: str, chapter_title: str, book_title: str, author: str, provider: str = "auto") -> str:
        """Generate a focused summary for a single chapter using a custom prompt."""
        return asyncio.run(self._generate_chapter_summary(content, chapter_title, book_title, author, provider))

    async def _generate_chapter_summary(self, content: str, chapter_title: str, book_title: str, author: str, provider: str = "auto") -> str:
        if not self.client:
...
"""
    ...
    async def generate_text_from_prompt(self, prompt: str, provider: str = "auto") -> str:
    ...
    async def generate_tutorial_script(self, prompt: str, provider: str = "auto") -> str:
    ...
    async def _generate_scene_descriptions(self, chapter_id: str, rag_service, provider: str = "auto") -> List[str]:
...
"""
    ...
    async def extract_real_chapters_from_list(self, prompt: str, provider: str = "auto") -> Dict[str, Any]:
