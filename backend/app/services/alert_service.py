...
class AlertService:
    ...
    async def get_alert_settings(self) -> Dict[str, Any]:
        ...
        query = self.supabase.table("admin_settings").select("*").eq(
    ...
    async def check_all_alerts(self) -> List[Dict[str, Any]]:
        ...
        fallback_alerts = await self.check_high_fallback_rates(settings["high_fallback_rate"])
        ...
        success_alerts = await self.check_low_success_rates(settings["low_success_rate"])
    ...
    async def check_high_fallback_rates(self, threshold: float = 30.0) -> List[Dict[str, Any]]:
        """Check if any service has high fallback rates"""
        alerts = []

        # Get fallback rates for last hour
        one_hour_ago = datetime.now() - timedelta(hours=1)
        fallback_data = await self.metrics_service.get_fallback_rates(start_date=one_hour_ago)

        # Check each service
        services = ["script_generation", "image_generation", "video_generation", "audio_generation"]
        ...
        if overall_rate > threshold:
            ...
            alerts.append({
                ...
                "metadata": {
                    "service": "all",
                    "total_operations": fallback_data["overall_summary"]["total_operations"],
                    "total_fallbacks": fallback_data["overall_summary"]["total_fallbacks"],
                    "period": "last_hour",
                },
            })
    ...
    async def check_cost_spikes(self, spike_percentage: float = 50.0) -> List[Dict[str, Any]]:
    ...
    async def check_low_success_rates(self, min_success_rate: float = 80.0) -> List[Dict[str, Any]]:
        """Check for models with low success rates"""
        alerts = []

        # Get model performance for last 6 hours
        six_hours_ago = datetime.now() - timedelta(hours=6)
        model_performance = await self.metrics_service.get_model_performance(start_date=six_hours_ago)
    ...
    async def create_alert(self, alert_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if existing_response.data:
            for existing in existing_response.data:
                if existing.get("metadata", {}).get("service") == alert_data.get("metadata", {}).get("service"):
                    return existing

        # Create new alert
        insert_response = self.supabase.table("admin_alerts").insert({
    ...
    async def get_recent_alerts(
        ...
        query = self.supabase.table("admin_alerts").select("*").gte(
    ...
    async def acknowledge_alert(self, alert_id: str, user_id: str) -> Dict[str, Any]:
        ...
        update_response = self.supabase.table("admin_alerts").update({
    ...
    async def update_alert_settings(
        ...
        upsert_response = self.supabase.table("admin_settings").upsert({
            "setting_key": "alert_thresholds",
