...
class ModelsLabV7VideoService:
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        processed_response = self._process_video_response(result, 'image_to_video')
                        ...
                        if processed_response.get('status') == 'processing':
                            ...
                            if fetch_result:
                                logger.info(f"[MODELSLAB V7 VIDEO] Starting polling for image-to-video completion")
                                poll_result = await self._poll_for_video_completion(fetch_result, future_links)
                                return poll_result
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    processed_response = self._process_video_response(result, 'lip_sync')
                    ...
                    if processed_response.get('status') == 'processing':
                        ...
                        future_links = processed_response.get('future_links', [])
                        if fetch_result:
                            logger.info(f"[MODELSLAB V7 VIDEO] Starting polling for lip sync completion")
                            poll_result = await self._poll_for_video_completion(fetch_result, future_links)
                            return poll_result
    ...
    async def enhance_video_for_scene(
        ...
        try:
            ...
            if dialogue_audio and len(dialogue_audio) > 0:
                logger.info(f"[SCENE VIDEO] Generating {len(dialogue_audio)} separate video scenes for dialogues")
    ...
    async def _generate_per_dialogue_videos(
        ...
        logger.info(f"[PER-DIALOGUE VIDEO] Generating {len(dialogue_audio)} separate video scenes")
        ...
        for i, dialogue in enumerate(dialogue_audio):
            ...
            logger.info(f"[PER-DIALOGUE VIDEO] Processing dialogue {i+1}: {character_name}")
            ...
            if video_result.get('status') != 'success':
                logger.error(f"[PER-DIALOGUE VIDEO] Video generation failed for dialogue {i+1}: {video_result.get('error')}")
                continue
            ...
            if include_lipsync and dialogue_audio_url:
                logger.info(f"[PER-DIALOGUE VIDEO] Applying lip sync for {character_name}")
                ...
                if lipsync_result.get('status') == 'success':
                    ...
                    if lipsync_video_url:
                        ...
                        logger.info(f"[PER-DIALOGUE VIDEO] ✅ Lip sync applied for {character_name}")
                    else:
                        logger.warning(f"[PER-DIALOGUE VIDEO] ⚠️ Lip sync completed but no video URL for {character_name}")
                else:
                    logger.warning(f"[PER-DIALOGUE VIDEO] ⚠️ Lip sync failed for {character_name}: {lipsync_result.get('error')}")
            ...
            logger.info(f"[PER-DIALOGUE VIDEO] ✅ Dialogue {i+1} video generated successfully")
...
"""
        ...
        logger.info(f"[DIALOGUE PROMPT] Created focused prompt for {character_name}: {enhanced_prompt[:200]}...")
...
""".strip()
    ...
    def _create_enhanced_video_prompt(
        ...
        logger.info(f"[ENHANCED PROMPT] Creating enhanced prompt for scene: {scene_description[:50]}...")
        ...
        if camera_movements:
            ...
            logger.info(f"[ENHANCED PROMPT] Added {len(camera_movements)} camera movements")
        ...
        if character_actions:
            ...
            logger.info(f"[ENHANCED PROMPT] Added {len(character_actions)} character actions")
        ...
        if character_dialogues:
            ...
            logger.info(f"[ENHANCED PROMPT] Added {len(character_dialogues)} character dialogues")
        
        # Add scene transitions
        if scene_transitions:
            transitions_text = "Scene transitions: " + ", ".join(scene_transitions)
            prompt_parts.append(transitions_text)
            logger.info(f"[ENHANCED PROMPT] Added {len(scene_transitions)} scene transitions")
        ...
        logger.info(f"[ENHANCED PROMPT] Final prompt length: {len(enhanced_prompt)} characters")
    ...
    def _create_enhanced_video_prompt_from_script(
        ...
        logger.info(f"[ENHANCED PROMPT] No parsed components available, using fallback method")
        return self._create_scene_video_prompt_with_characters(scene_description, style, dialogue_audio, script_style)
        ...
        for field in url_fields:
            ...
            if urls and len(urls) > 0:
                ...
                if video_url and isinstance(video_url, str):
                    logger.info(f"[MODELSLAB V7 VIDEO] Extracted video URL from {field}: {video_url}")
                    return video_url
        
        logger.warning(f"[MODELSLAB V7 VIDEO] No video URL found in response fields: {list(response.keys())}")
        return None

    async def _try_fallback_retrieval(self, future_links: List[str]) -> Optional[Dict[str, Any]]:
        """Attempt fallback video retrieval using future_links URLs"""
        
        if not future_links:
            return None
            
        logger.info(f"[MODELSLAB V7 VIDEO] Attempting fallback retrieval from {len(future_links)} future_links")
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.head(
                        ...
                        if response.status == 200:
                            ...
                            if 'video' in content_type.lower() or 'mp4' in video_url.lower():
                                logger.info(f"[MODELSLAB V7 VIDEO] ✅ Fallback video found: {video_url} (size: {content_length} bytes)")
                                return {
                                    'status': 'success',
                                    'video_url': video_url,
                                ...
                                logger.warning(f"[MODELSLAB V7 VIDEO] Fallback URL not a video: {content_type}")
                        else:
                            logger.warning(f"[MODELSLAB V7 VIDEO] Fallback URL unavailable: HTTP {response.status}")
                            
            except Exception as e:
                logger.warning(f"[MODELSLAB V7 VIDEO] Fallback retrieval failed for {video_url}: {str(e)}")
                continue
    ...
    async def _poll_for_video_completion(
        ...
        logger.info(f"[MODELSLAB V7 VIDEO] Starting polling for video completion: {fetch_result_url}")
        ...
        while (asyncio.get_event_loop().time() - start_time) < max_poll_time:
            poll_count += 1
            logger.info(f"[MODELSLAB V7 VIDEO] Poll attempt {poll_count}, delay: {current_delay}s")
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            logger.info(f"[MODELSLAB V7 VIDEO] Poll response status: {status}")
                            
                            if status == 'success':
                                video_url = self._extract_video_url(result)
                                if video_url:
                                    logger.info(f"[MODELSLAB V7 VIDEO] ✅ Video generation completed successfully")
                                    return {
                                        'status': 'success',
                                        'video_url': video_url,
                                    ...
                                    logger.warning(f"[MODELSLAB V7 VIDEO] Success status but no video URL found")
                                ...
                                logger.info(f"[MODELSLAB V7 VIDEO] Still processing, ETA: {eta}s")
                                ...
                                error_msg = result.get('message', result.get('error', 'Unknown error during polling'))
                                logger.error(f"[MODELSLAB V7 VIDEO] Polling error: {error_msg}")
                                # Try fallback retrieval if available
                                if future_links:
                                    fallback_result = await self._try_fallback_retrieval(future_links)
                                    if fallback_result:
                            ...
                            error_text = await response.text()
                            logger.error(f"[MODELSLAB V7 VIDEO] HTTP {response.status} during polling: {error_text}")
    ...
    def _process_video_response(self, response: Dict[str, Any], operation_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response_status == 'processing':
                logger.info(f"[MODELSLAB V7 VIDEO] Video generation in progress, status: processing")
    ...
    async def retry_video_retrieval(self, video_url: str) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"[VIDEO RETRY] Attempting video retrieval from URL: {video_url}")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    async with session.head(
                        ...
                        if not any(video_type in content_type for video_type in ['video', 'mp4', 'mov', 'avi']):
                            logger.warning(f"[VIDEO RETRY] Content type may not be video: {content_type}")
                            # Continue anyway as some video URLs might not have proper content-type
                
                except Exception as head_error:
                    logger.warning(f"[VIDEO RETRY] HEAD request failed: {str(head_error)}")
                ...
                try:
                    ...
                    async with session.get(
                        ...
                        headers={'Range': 'bytes=0-1024'}  # Download first 1KB to validate
                    ) as get_response:
                        
                        if get_response.status not in [200, 206]:  # 206 is partial content
                            return {
                                'success': False,
                        ...
                        is_video_file = any(video_data.startswith(sig) for sig in video_signatures)
                        
                        if not is_video_file:
                            logger.warning(f"[VIDEO RETRY] File doesn't appear to be a video (no recognized signature)")
                            # Continue anyway as some video files might not have standard signatures
                
                except Exception as validation_error:
                    logger.warning(f"[VIDEO RETRY] Video validation failed: {str(validation_error)}")
                ...
                try:
                    ...
                    async with session.get(
                        video_url,
                        timeout=aiohttp.ClientTimeout(total=300)  # 5 minutes for full download
                    ) as full_response:
                        ...
                        content_length = full_response.headers.get('content-length', '0')
                        ...
                        logger.info(f"[VIDEO RETRY] Content-Length: {content_length} bytes")
                    ...
                    logger.error(f"[VIDEO RETRY] Full video download failed: {str(download_error)}")
                    return {
                        'success': False,
            ...
            logger.error(f"[VIDEO RETRY] Unexpected error during video retrieval: {str(e)}")
            return {
                'success': False,
