...
class UserSubscription(SQLModel, table=True):
    ...
    # DEPRECATED: credit-only migration (KAN-265) — monthly caps removed, credit gating only
    monthly_video_limit: int = Field(default=0)  # DEPRECATED: no longer used for gating
    video_quality: str = Field(default="720p")
    has_watermark: bool = Field(default=True)
    videos_generated_this_period: int = Field(default=0)  # DEPRECATED: no longer used for gating
