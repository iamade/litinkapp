...
def generate_quiz_task(self, content: str, difficulty: str = "medium"):
    ...
    try:
        current_task.update_state(
            state="PROGRESS", meta={"status": "Generating quiz questions..."}
        )
        ...
        return {"status": "SUCCESS", "questions": quiz_questions}
...
def generate_video_task(
    ...
    try:
        current_task.update_state(
            state="PROGRESS", meta={"status": "Generating video scene..."}
        )

        video_service = VideoService()
        # Sync version needed for Celery
        video_scene = video_service.generate_story_scene_sync(
            scene_description, dialogue, avatar_style
        )

        return {"status": "SUCCESS", "video_scene": video_scene}
    except Exception as e:
        return {"status": "FAILURE", "error": str(e)}
...
def process_book_upload_task(self, file_path: str, book_data: dict):
    ...
    try:
        current_task.update_state(
            state="PROGRESS", meta={"status": "Processing book content..."}
        )
        ...
        return {"status": "SUCCESS", "book_id": "generated_book_id"}
    except Exception as e:
        return {"status": "FAILURE", "error": str(e)}
