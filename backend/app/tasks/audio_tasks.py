    ...
    for i, dialogue in enumerate(character_dialogues):
        try:
            ...
            result = await audio_service.generate_tts_audio(
                ...
                voice_id=voice_id,
                speed=1.0,
                emotion="yes"  # ✅ FIXED: Use "yes" instead of boolean
            )
...
async def generate_background_music(
    ...
    for i, music_cue in enumerate(music_cues):
        try:
            ...
            # ✅ FIXED: Limit duration to maximum 15 seconds (API limit)
            duration = min(15.0, music_cue.get('duration', 15.0))
            
            # Generate background music using sound effect API
            result = await audio_service.generate_sound_effect(
                description=music_cue['description'],
                duration=duration  # ✅ FIXED: Use limited duration
            )
            ...
            if result.get('status') == 'success':
                ...
                if output_urls:
                    audio_url = output_urls[0]
                    duration = result.get('audio_time', duration) or duration
