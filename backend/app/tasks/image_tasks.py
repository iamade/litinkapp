...
def generate_all_images_for_video(self, video_generation_id: str):
    ...
    try:
        ...
        usage_check = asyncio.run(
            subscription_manager.check_usage_limits(user_id, "image")
        )
        ...
        if characters:
            ...
            character_images = asyncio.run(
                generate_character_images_optimized(
                    image_service,
                    video_generation_id,
                    characters,
                    video_style,
        ...
        if scene_descriptions:
            ...
            scene_images = asyncio.run(
                generate_scene_images_optimized(
        ...
        supabase.table("video_generations").update(
            {"image_data": image_data, "generation_status": "images_completed"}
        ).eq("id", video_generation_id).execute()
...
def generate_character_image_task(
    ...
    try:
        ...
        supabase.table("image_generations").update(update_data).eq(
            "id", record_id
        ).execute()
        logger.info(
        ...
        if character_id and image_url:
            try:
                character_update_data = {
                    ...
                    "image_generation_prompt": result.get(
                        "prompt_used",
                        custom_prompt or f"Character portrait: {character_name}",
                    ),
                    "image_metadata": {
                ...
                supabase.table("characters").update(character_update_data).eq(
                    "id", character_id
                ).execute()
                logger.info(
...
def generate_scene_image_task(
    ...
    try:
        ...
        result = asyncio.run(
            ModelsLabV7ImageService().generate_scene_image(
        ...
        if is_retryable and retry_count < 3:
            ...
            try:
                ...
                logger.error(
                    f"[SceneImageTask] Failed to increment retry_count: {retry_db_error}"
                )
