...
async def update_merge_progress(
    ...
    try:
        ...
        # We assume MergeOperation model exists (need to check, but for now using raw SQL or generic update if model not available)
...
def get_quality_settings(
    quality_tier: MergeQualityTier, custom_params: Optional[FFmpegParameters] = None
) -> Dict[str, Any]:
    ...
    settings = base_settings.get(
        quality_tier, base_settings[MergeQualityTier.WEB]
    ).copy()
...
async def async_merge_audio_video_for_generation(video_generation_id: str):
    ...
    async with async_session() as session:
        try:
            ...
            if video_gen.generation_status != "video_completed":
                raise Exception("Video generation must be completed before merging")

            # Update status
            video_gen.generation_status = "merging_audio"
            session.add(video_gen)
            await session.commit()
            await session.refresh(video_gen)

            # Get all necessary data
            audio_files = video_gen.audio_files or {}
            video_data_obj = video_gen.video_data or {}
            ...
            for idx, scene in enumerate(scene_videos):
                ...
                if not scene_id:
                    print(
                        f"[MERGE] Skipping scene with missing scene_id at index {idx}: {scene}"
                    )
                    continue
            ...
            print(
                f"- Total duration: {merge_statistics.get('total_duration', 0):.1f} seconds"
            )
            print(f"- File size: {merge_statistics.get('file_size_mb', 0):.1f} MB")
            print(
                f"- Processing time: {merge_statistics.get('processing_time', 0):.1f} seconds"
            )
            print(
                f"- Audio track mixing: {merge_statistics.get('audio_tracks_mixed', 0)} tracks"
            )
            print(
                f"- Synchronization accuracy: {merge_statistics.get('sync_accuracy', 'N/A')}"
            )
            ...
            try:
                # Re-fetch to ensure attached to session if needed, or just update
...
async def merge_audio_video_scenes(
    ...
    try:
        ...
        audio_preparation_result = await prepare_audio_tracks(
            audio_files, valid_scene_videos
        )
        ...
        for i, scene_video in enumerate(valid_scene_videos):
            try:
                print(
                    f"[SCENE MERGE] Processing scene {i+1}/{len(valid_scene_videos)}: {scene_video.get('scene_id')}"
                )
                ...
                if merged_scene:
                    ...
                    print(
                        f"[SCENE MERGE] ✅ Scene {scene_video.get('scene_id')} merged successfully"
                    )
                else:
                    print(
                        f"[SCENE MERGE] ⚠️ Scene {scene_video.get('scene_id')} merge failed, using original video"
                    )
                ...
                print(
                    f"[SCENE MERGE] ❌ Scene {scene_video.get('scene_id')} failed: {str(e)}"
                )
        ...
        audio_tracks_mixed = len(audio_files.get("narrator", [])) + len(
...
async def prepare_audio_tracks(
    audio_files: Dict[str, Any], scene_videos: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    for audio in narrator_files:
        ...
        scene_id = (
            audio.get("scene")
            or audio.get("scene_id")
            or audio.get("metadata", {}).get("scene")
        )
        if scene_id:
    ...
    print(
        f"[AUDIO PREP] Organized {total_audio_files} audio files across {len(scene_audio_tracks)} scenes"
    )
    for scene_id, tracks in scene_audio_tracks.items():
        ...
        print(
            f"[AUDIO PREP] Scene {scene_id}: {narrator_count} narrator, {character_count} character, {sfx_count} sfx files"
        )
...
def find_audio_for_scene(
    scene_id: str, scene_audio_tracks: Dict[str, Any]
) -> Dict[str, Any]:
    """Find audio tracks for a specific scene"""

    return scene_audio_tracks.get(
        scene_id, {"narrator": [], "characters": [], "sound_effects": []}
    )


async def merge_single_scene(
    scene_video: Dict[str, Any], scene_audio: Dict[str, Any], video_generation_id: str
    ...
    if not (has_narrator or has_characters or has_sound_effects):
        print(
            f"[SINGLE SCENE MERGE] No audio for scene {scene_id}, using original video"
        )
        return {
            "scene_id": scene_id,
            "video_url": video_url,
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not audio_files_paths:
                print(
                    f"[SINGLE SCENE MERGE] No valid audio files downloaded for scene {scene_id}"
                )
                return {
                    "scene_id": scene_id,
                    "video_url": video_url,
            ...
            await merge_audio_with_video_ffmpeg(
                video_path, mixed_audio_path, output_path
            )
...
async def merge_audio_with_video_ffmpeg(
...
async def generate_scene_transition(
    ...
    try:
        ...
        if not key_shot_url:
            print(
                f"[TRANSITION] No key scene shot for scene {scene_a.get('scene_id')}, skipping transition"
            )
            return None

        # Download key scene shot
        key_shot_path = os.path.join(
            temp_dir, f"key_shot_{scene_a.get('scene_id')}.jpg"
        )
        ...
        first_frame_path = os.path.join(
            temp_dir, f"first_frame_{scene_b.get('scene_id')}.jpg"
        )
        cmd_extract = [
            "ffmpeg",
            "-y",
        ...
        transition_output = os.path.join(
            temp_dir,
            f"transition_{scene_a.get('scene_id')}_to_{scene_b.get('scene_id')}.mp4",
        )
        ...
        cmd_transition = [
            ...
            "1",
            "-i",
            key_shot_path,  # Loop key shot
            "-loop",
            "1",
            "-i",
            first_frame_path,  # Loop first frame
            "-filter_complex",
        ...
        print(
            f"[TRANSITION] ✅ Created {transition_duration}s transition between scenes"
        )
...
async def apply_video_filters(
    input_video: str, output_video: str, filters: List[str] = None
) -> bool:
    """Apply advanced video filters using FFmpeg"""

    if not filters:
        filters = [
            "colorlevels=rimin=0.058:gimin=0.058:bimin=0.058:rimax=0.898:gimax=0.898:bimax=0.898"
...
async def reencode_video(
    ...
    try:
        ...
        print(
            f"[REENCODE] Re-encoding with {quality_tier.value} quality to {format_type}"
        )
        import time
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, scene in enumerate(merged_scenes):
                ...
                if video_url:
                    ...
                    if i < len(merged_scenes) - 1:
                        ...
                        transition_path = await generate_scene_transition(
                            scene, next_scene, temp_dir
                        )
                        if transition_path:
            ...
            cmd_concat = [
                ...
                concat_file,
                "-c",
                "copy",  # Copy without re-encoding first
                raw_concat_output,
            ]

            print(
                f"[FINAL CONCAT] Concatenating {len(sequence_files)} segments (scenes + transitions)"
            )
            import time
            ...
            filters_applied = await apply_video_filters(
                raw_concat_output, filtered_output
            )
            ...
            for preset in quality_presets:
                quality_output = os.path.join(
                    temp_dir, f"final_video_{preset.value}.mp4"
                )
                reencode_result = await reencode_video(
                    processing_input, quality_output, preset, "mp4"
                )
            ...
            for version in quality_versions:
                quality_file = os.path.join(
                    temp_dir, f"final_video_{version['quality']}.mp4"
                )
            ...
            print(
                f"[FINAL CONCAT] ✅ Final video created: {file_size_mb:.1f}MB in {processing_time:.1f}s with {len(quality_versions)} quality versions"
            )
        ...
        "quality_options": [
            {
                ...
                "recommended_for": (
                    "web"
                    if v.get("quality") == "web"
                    else ("high_quality" if v.get("quality") == "high" else "standard")
                ),
            }
            for v in quality_versions
            if v.get("url")
        ],
        "technical_specs": {
...
async def async_process_manual_merge(merge_id: str, user_id: str):
    ...
    async with async_session() as session:
        try:
            ...
            # merge_record is a RowMapping, access like dict
            merge_data = {
            ...
            # Assuming generate_merge_preview is imported or available via celery_app
            # It seems it was called as generate_merge_preview.delay(merge_id) in original code
            # But I don't see generate_merge_preview imported. It might be defined in this file later?
            # Ah, process_merge_preview is defined later. Maybe generate_merge_preview is an alias or imported?
            # I'll check if generate_merge_preview is defined in the file.
            # If not, I'll assume it's process_merge_preview.delay or similar.
            # Wait, original code had `generate_merge_preview.delay(merge_id)`.
            # I'll keep it as is, assuming it's available.
            # Actually, I should check if it's imported. It wasn't in imports.
            # Maybe it's defined in the file but I missed it?
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if len(prepared_inputs) == 1:
                ...
                if merge_id:
                    await update_merge_progress(
                        merge_id, 40.0, "Concatenating multiple inputs", session=session
                    )
                merged_output = await concatenate_manual_inputs(
                    prepared_inputs,
                    temp_dir,
                    quality_tier,
                    output_format,
                ...
                if merge_id:
                    await update_merge_progress(
                        merge_id, 70.0, "Multiple inputs concatenated", session=session
                    )
...
async def prepare_manual_inputs(
    input_sources: List[Dict[str, Any]], temp_dir: str
) -> List[Dict[str, Any]]:
    ...
    for i, source in enumerate(input_sources):
        ...
        file_ext = (
            ...
            else ("mp3" if source_type == "audio" else "jpg")
        )
...
async def perform_merge_preview(
    preview_data: Dict[str, Any], user_id: str
) -> Dict[str, Any]:
    ...
    preview_duration = min(
        preview_data.get("preview_duration", 30.0), 30.0
    )  # Max 30 seconds
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            prepared_inputs = await prepare_preview_inputs(
                input_sources, temp_dir, preview_duration
            )
            ...
            if len(prepared_inputs) == 1:
                ...
                preview_output = await concatenate_preview_segments(
                    prepared_inputs, temp_dir, quality_tier, ffmpeg_params
                )
...
async def async_generate_merge_preview(merge_id: str):
    ...
    async with async_session() as session:
        try:
            ...
            # Using raw SQL
            query = text("SELECT * FROM merge_operations WHERE id = :id")
            ...
            if merge_record.get("merge_status") != "COMPLETED":
                raise Exception(f"Merge operation {merge_id} is not completed yet")

            output_file_url = merge_record.get("output_file_url")
            if not output_file_url:
                raise Exception(f"Merge operation {merge_id} has no output file URL")

            # Generate preview
            preview_url = await generate_video_preview_clip(output_file_url, merge_id)
            ...
            print(
                f"[MERGE PREVIEW] Successfully generated preview for merge {merge_id}: {preview_url}"
            )
...
async def generate_video_preview_clip(video_url: str, merge_id: str) -> Optional[str]:
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                f"[PREVIEW CLIP] Generated preview: {file_size_mb:.1f}MB, URL: {preview_url}"
            )
            return preview_url
