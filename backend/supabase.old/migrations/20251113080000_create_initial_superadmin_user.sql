...
BEGIN
  ...
  IF profile_exists THEN
    ...
    EXECUTE format('
      ...
      %s
      WHERE email = $1',
      CASE WHEN account_status_exists THEN ', account_status = ''active''' ELSE '' END,
      CASE WHEN is_active_exists THEN ', is_active = true' ELSE '' END
    ) USING superadmin_email;
  ...
  ELSE
    ...
    column_list := 'id, email, display_name, roles, email_verified, email_verified_at, created_at, updated_at';
    value_list := '$1, $2, $3, $4, $5, $6, $7, $8';

    IF account_status_exists THEN
      column_list := column_list || ', account_status';
    ...
    -- Create the profile with dynamic column list
    IF account_status_exists AND is_active_exists THEN
      ...
      ) USING superadmin_user_id, superadmin_email, 'LitInk Support',
              ARRAY['superadmin', 'creator', 'explorer']::text[],
              true, now(), now(), now(), 'active', true;
