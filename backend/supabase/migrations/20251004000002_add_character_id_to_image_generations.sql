-- Add character_id column to image_generations table
ALTER TABLE image_generations ADD COLUMN character_id UUID;

-- Add foreign key constraint with ON DELETE SET NULL
ALTER TABLE image_generations
ADD CONSTRAINT fk_image_generations_character_id
FOREIGN KEY (character_id)
REFERENCES characters(id)
ON DELETE SET NULL;

-- Create index for character_id
CREATE INDEX idx_image_generations_character_id ON image_generations(character_id);

-- Down migration
-- Drop index
DROP INDEX idx_image_generations_character_id;

-- Drop foreign key constraint
ALTER TABLE image_generations DROP CONSTRAINT fk_image_generations_character_id;

-- Remove column
ALTER TABLE image_generations DROP COLUMN character_id;