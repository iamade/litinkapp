...
ENV_FILE="$ENV_FILE" docker compose -p vps-staging -f local.yml up --build -d --no-deps api

echo "  ⏳ Waiting for API startup..."
for i in {1..60}; do
  logs=$(ENV_FILE="$ENV_FILE" docker compose -p vps-staging -f local.yml logs --no-color --tail=120 api 2>/dev/null || true)
...
ENV_FILE="$ENV_FILE" docker compose -p vps-staging -f local.yml up --build -d --no-deps celeryworker celerybeat
