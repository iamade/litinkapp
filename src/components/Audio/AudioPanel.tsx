...
const AudioPanel: React.FC<AudioPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'dialogue' | 'narration' | 'music' | 'effects' | 'ambiance' | 'timeline'>('dialogue');
  ...
  const [generationOptions, setGenerationOptions] = useState({
    voiceModel: 'elevenlabs_multilingual',
  ...
  const audioTabs = [
    { id: 'dialogue', label: 'Dialogue', icon: Mic, count: audioAssets.dialogue.length },
    { id: 'narration', label: 'Narration', icon: Volume2, count: audioAssets.narration.length },
    { id: 'music', label: 'Music', icon: Music, count: audioAssets.music.length },
    { id: 'effects', label: 'Effects', icon: Headphones, count: audioAssets.effects.length },
    { id: 'ambiance', label: 'Ambiance', icon: Wind, count: audioAssets.ambiance.length },
  ...
  const handleGenerateAll = async () => {
    ...
    if (!confirm(`Generate audio for all ${scenes.length} scenes? This may take several minutes.`)) {
      return;
    }
      ...
      <div className="bg-white border rounded-lg p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            >
              <option value="elevenlabs_multilingual">ElevenLabs Multilingual</option>
        ...
        <div className="mt-4 space-y-3">
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={generationOptions.generateDialogue}
              onChange={(e) => setGenerationOptions(prev => ({ ...prev, generateDialogue: e.target.checked }))}
              className="rounded border-gray-300 text-purple-600"
            />
            <span className="ml-2 text-sm text-gray-700">Generate character dialogue</span>
          </label>
...
const AudioFileCard: React.FC<{
  ...
  return (
    <div className={`bg-white border rounded-lg p-4 ${isSelected ? 'border-purple-500' : ''}`}>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Scene {file.sceneNumber} • {formatTime(file.duration)}
